/**
 * Seed Script — Populate PostgreSQL Apache AGE from embedded graph data
 *
 * Populates all IoT Cybersecurity vertices (nodes) and directed edges (relationships).
 *
 * Usage: npx tsx scripts/seed-age.ts
 */

import { config } from 'dotenv';
config({ path: '.env.local' });

import { runAgeQuery, closeAgePool } from '../lib/age';
...
async function main() {
  console.log('🌱 Seeding Apache AGE from embedded graph data...\n');
  console.log(`  📦 Entities to seed: ${nodes.length}`);

  // 1. MERGE all nodes with full properties
  console.log('  ⏳ Merging nodes into Apache AGE...');
  for (const n of nodes) {
    const cypher = `
      MERGE (n:Entity {id: '${escapeStr(n.id)}'})
      SET n.label = '${escapeStr(n.label)}',
          n.kind = '${escapeStr(n.kind)}',
          n.val = ${n.val},
          n.color = '${escapeStr(n.color)}',
          n.status = '${escapeStr(n.status)}',
          n.detail = '${escapeStr(n.detail)}'
      RETURN n
    `;
    await runAgeQuery(cypher, 'n agtype');
  }
  console.log(`  ✅ All ${nodes.length} nodes merged.`);

  // 2. Build and merge relationships
  const nodeKindMap = new Map(nodes.map((n) => [n.id, n.kind]));
  console.log('  ⏳ Merging edges into Apache AGE...');

  let edgeCount = 0;
  for (const l of links) {
    const relType = inferRelType(nodeKindMap.get(l.source) ?? 'agent', nodeKindMap.get(l.target) ?? 'tool');
    const cypher = `
      MATCH (a:Entity {id: '${escapeStr(l.source)}'}), (b:Entity {id: '${escapeStr(l.target)}'})
      CREATE (a)-[:${relType}]->(b)
      RETURN a
    `;
  ...
  console.log(`  ✅ All ${edgeCount} relationships created.`);
  console.log('\n🎉 Apache AGE Graph Migration Complete!');
